...
class AldousBroderAlgorithm(protected val width: Int, protected val height: Int, protected val seed: Option[Long]= None) extends MazeGenerationAlgorithm {
  ...
  private def go(x: Int, y: Int): Unit = {
    if (grid.flatten.exists(_.hasAllWalls)) {
      ...
      if (nextCell.hasAllWalls) cell.knockDownWall(nextCell, direction)
